...
//   • data/CloudApi.kt        — REAL (Firestore) vs FAKE (offline) cloud + the switch
...
import com.example.firebasesync.ui.theme.FirebaseSyncTheme
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            FirebaseSyncTheme {
                val vm: FirebaseSyncViewModel = viewModel(factory = FirebaseSyncViewModel.factory(application))
                FirebaseSyncApp(vm)
            }
        }
    }
...
@Composable
fun FirebaseSyncApp(vm: FirebaseSyncViewModel) {
    ...
    Scaffold(
        ...
        topBar = { TopAppBar(title = { Text("FirebaseSync") }) },
    ) { inner ->
