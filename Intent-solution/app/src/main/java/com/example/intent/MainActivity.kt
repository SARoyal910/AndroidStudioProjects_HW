...
data class DetailKey(val message: String, val count: Int) : NavKey  // B2: second typed argument `count` alongside `message`.
        ...
        entryProvider = entryProvider {                   // Begin the map of key type -> screen UI.
            entry<HomeKey> {                              // If the top key is a HomeKey, show this:
                HomeScreen(                               // Draw the Home screen.
                    ...
                    onOpenDetail = { backStack.add(DetailKey("Hello from Home!", 3)) }  // B2: pass a count too.
                )                                         // End HomeScreen call.
            }                                             // End HomeKey entry.
            entry<DetailKey> { key ->                     // If the top key is a DetailKey, receive it as `key` and show:
                DetailScreen(                             // Draw the Detail screen.
                    ...
                    count = key.count,                    // B2: forward the second argument too.
...
@Composable                                               // Compose UI function.
fun DetailScreen(message: String, count: Int, onBack: () -> Unit, modifier: Modifier = Modifier) {  // B2: now also takes `count`.
    Column(modifier = modifier) {                         // Stack the children vertically.
        Text(text = "$message ($count)")                  // B2: show "message (count)", e.g. "Hello from Home! (3)".
...
fun DetailScreenPreview() {                               // A preview wrapper for DetailScreen.
    IntentTheme {                                         // Apply the theme.
        DetailScreen(message = "Preview message", count = 1, onBack = {})  // B2: preview now supplies a count.
    }                                                     // End IntentTheme.
