...
// ===========================================================================
// MAPPER  —  the SINGLE place wire-shape becomes app-shape   [SOLUTION — Part B4]
//
// These four functions are the whole "parsing" payoff: they take the raw network
// shape and quirks (a number hidden in a URL, decimetres, hectograms, a nested
// type wrapper) and produce clean domain values the UI can render directly. They
// are PURE (no Android, no I/O), which is exactly why Part G can unit-test them.
// ===========================================================================

/**
 * Parse the trailing Pokédex number out of a PokéAPI resource url.
 *   "https://pokeapi.co/api/v2/pokemon/25/" -> 25
 *
 * trimEnd('/') drops the trailing slash, substringAfterLast('/') grabs the last
 * path segment, and toIntOrNull() turns it into an Int (falling back to 0 if the
 * url is ever malformed, so a bad row can never crash the list).
 */
fun dexNumberFromUrl(url: String): Int =
    url.trimEnd('/').substringAfterLast('/').toIntOrNull() ?: 0

/**
 * Map a PokéAPI type NAME to an emoji glyph for the UI (the course's
 * no-icon-library convention). The `else` branch is the important part: an
 * unknown/new type returns a neutral glyph instead of crashing.
 */
fun typeEmoji(typeName: String): String = when (typeName) {
    "normal" -> "⚪"
    "fire" -> "🔥"
    "water" -> "💧"
    "grass" -> "🌿"
    ...
    "steel" -> "⚙️"
    "fairy" -> "🧚"
    else -> "❓"                                // unknown type: degrade, don't crash
}

/** Flatten one list entry into a [PokemonSummary] (dex number parsed from the url). */
fun NamedResourceDto.toSummary(): PokemonSummary =
    PokemonSummary(dexNumber = dexNumberFromUrl(url), name = name)

/**
 * Flatten the detail DTO into a [PokemonDetail]:
 *   • dexNumber = id
 *   • types     = each types[].type.name -> PokemonType(name, its emoji)
 *   • height/10.0 -> metres (the wire value is DECIMETRES)
 *   • weight/10.0 -> kilograms (the wire value is HECTOGRAMS)
 */
fun PokemonDetailDto.toDetail(): PokemonDetail =
    PokemonDetail(
        dexNumber = id,
        name = name,
        types = types.map { PokemonType(it.type.name, typeEmoji(it.type.name)) },
