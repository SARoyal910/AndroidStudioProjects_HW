...
interface PokemonDao {
    ...
    /** Insert-or-replace ONE full row (used by the detail fetch to hydrate a row). */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsert(pokemon: CachedPokemon)

    /**
     * Insert summary rows but DO NOT touch rows that already exist (IGNORE).
     *
     * This is the key to refreshing the list without losing detail: the list
     * endpoint only knows name + dexNumber, so inserting those with REPLACE would
     * overwrite any already-hydrated row and NULL out its height/weight/types.
     * IGNORE adds genuinely new Pokémon and leaves existing (possibly detailed)
     * rows alone. Detail fetches still use [upsert] (REPLACE) to write the full row.
     */
    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun insertNewSummaries(pokemon: List<CachedPokemon>)
}
