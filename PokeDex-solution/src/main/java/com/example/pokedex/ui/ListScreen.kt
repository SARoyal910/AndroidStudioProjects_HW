...
import com.example.pokedex.providePokemonRepository

@OptIn(ExperimentalMaterial3Api::class)             // TopAppBar is still an experimental Material 3 API
@Composable
fun ListScreen(onOpen: (String) -> Unit) {
    // Build the real repository (it needs a Context to open Room) and inject it into
    // the VM via its factory. remember(context) keeps one instance across recomposition.
    val context = LocalContext.current
    val repository = remember(context) { providePokemonRepository(context) }
    val viewModel: PokemonListViewModel = viewModel(factory = PokemonListViewModel.factory(repository))
